#!/usr/bin/env node
/**
 * HarzFishing – Review Resolver v7.1.1
 *
 * Zweck:
...
const REPORT = path.join(DATA, "review-v7-1-1-result.csv");
...
function render(index) {
  return `// AUTO-GENERATED / updated by scripts/review-resolver-v7-1-1.mjs
...
// Cluster-Key: amtliche Feature-ID kommt aus dem kumulativen Index.
// Der v7-Bericht enthält diese Spalte nicht zuverlässig.
const groups=new Map();
for (const r of open) {
  const cur=index[r.id] || {};
  const fid=String(cur.officialFeatureId||"").trim();
  if (!fid) continue;
...
for (const r of open) {
  const cur=index[r.id];
  const fid=String(cur.officialFeatureId||"").trim();
  const dist=n(cur.officialDistanceM ?? r.officialDistanceM);
  const count=n(cur.officialCandidateCount ?? r.officialCandidateCount);
  const gap=n(cur.distanceGapM ?? r.distanceGapM);
  const officialName=String(cur.officialName ?? r.officialName ?? "");
  const ns=Math.max(n(cur.nameScore ?? r.nameScore)??0, nameSim(r.name,officialName));
  ...
  if (!fid || dist==null) {
    ...
    const ranked=[...g].sort((a,b)=>
      (n(index[a.id]?.officialDistanceM ?? a.officialDistanceM)??1e9) -
      (n(index[b.id]?.officialDistanceM ?? b.officialDistanceM)??1e9)
    );
    const best=ranked[0], second=ranked[1];
    const bestDist=n(index[best.id]?.officialDistanceM ?? best.officialDistanceM)??1e9;
    const secondDist=second
      ? (n(index[second.id]?.officialDistanceM ?? second.officialDistanceM)??1e9)
      : 1e9;
  ...
  report.push([
    ...
    fid,dist,officialName,count,ns.toFixed(3),gap,
    featureUseCount,reason
  ]);
}
...
console.log("");
console.log("HarzFishing – Review Resolver v7.1.1");
console.log("---------------------------------");
