#!/usr/bin/env node
/**
 * HarzFishing – Zwei-Quellen-Matcher v6.3
 *
...
function renderTs(index) {
  return `// AUTO-GENERATED by scripts/verify-two-source-v6-3.mjs
...
// v6.3 ist kumulativ: vorhandene Ergebnisse bleiben erhalten.
// Dadurch überschreibt Paket 51-100 nicht mehr Paket 1-50.
let existingRecords = {};
try {
  const existingText = await fs.readFile(OUTPUT, "utf8");
  existingRecords = extractAssignedJson(
    existingText,
...
console.log("");
console.log("HarzFishing – Zwei-Quellen-Matcher v6.3");
console.log("---------------------------------------");
console.log(`Bearbeitung: ${start}–${start + work.length - 1}`);
console.log(`Amtliche BBOX: ±${bboxRadiusM} m`);
console.log(`OSM-v3 Datensätze vorhanden: ${Object.keys(v3).length}`);
console.log(`Bereits kumuliert vorhanden: ${Object.keys(existingRecords).length}`);
console.log("");
...
for (let i = 0; i < work.length; i++) {
  ...
  if (
    ...
    batchStats.unmatched = (batchStats.unmatched || 0) + 1;
  ...
  try {
    ...
    batchStats.review = (batchStats.review || 0) + 1;

    await fs.writeFile(OUTPUT, renderTs(records), "utf8");
    continue;
  }
  ...
  batchStats[record.status] = (batchStats[record.status] || 0) + 1;
...
// Immer auch dann speichern, wenn ein komplettes Paket "unmatched" war.
await fs.writeFile(OUTPUT, renderTs(records), "utf8");

const reviewRows = [reviewHeader];
...
console.log("");
console.log("Paket-Ergebnis:", batchStats);
console.log("Kumuliert:", stats);
console.log(
