...
const nearRadiusM = Math.max(800, Math.min(radiusM, Number(option("near-radius", "2800")) || 2800));
const overpassTimeoutSec = Math.max(10, Math.min(45, Number(option("overpass-timeout", "25")) || 25));
const requestTimeoutMs = Math.max(12000, (overpassTimeoutSec + 4) * 1000);
const email = option("email", process.env.NOMINATIM_EMAIL || "");
const userAgent = `HarzFishingNavigator-local-water-matcher/3.1-fast${email ? ` (${email})` : ""}`;
...
function overpassQuery(latitude, longitude, radius, water) {
  // v3.1: Nur die erwartete Gewässerklasse abfragen.
  // Das reduziert die Last auf öffentlichen Overpass-Servern deutlich.
  if (expectedClass(water) === "flow") {
...
async function overpassNearby(anchor, water, cache, radius) {
  const key =
    `water-fast-v31|${expectedClass(water)}|` +
    `${anchor.latitude.toFixed(5)}|${anchor.longitude.toFixed(5)}|${radius}`;

  if (key in cache.overpass) return cache.overpass[key];

  const query = overpassQuery(anchor.latitude, anchor.longitude, radius, water);
  let lastError;

  // v3.1: Jeder Server bekommt nur EINEN Versuch.
  // Bei 429/502/503/504 oder Timeout sofort zum nächsten Endpoint.
  for (const endpoint of OVERPASS_ENDPOINTS) {
    try {
      const data = await fetchOverpassOnce(endpoint, query);
      ...
      const short =
        error?.name === "AbortError"
          ? `Timeout nach ~${Math.round(requestTimeoutMs / 1000)} s`
          : error?.message || "unbekannter Fehler";

      console.warn(`Overpass-Server übersprungen: ${short}`);

      // Keine langen Retry-Pausen mehr.
      await sleep(250);
    }
  }

  throw new Error(`alle Overpass-Server fehlgeschlagen: ${lastError?.message || "unknown"}`);
}

function shouldExpandSearch(match, candidates, nearRadius) {
  if (!candidates.length) return true;
  if (!match?.candidate) return true;

  // Gute nahe Kandidaten nicht durch eine teure 6-km-Abfrage verzögern.
  if (match.status === "matched") return false;

  const namedPlausible =
    match.candidate.names?.length > 0 &&
    match.name >= 0.65 &&
    match.distanceKm <= Math.min(2.2, nearRadius / 1000);

  const unnamedPlausible =
    match.candidate.names?.length === 0 &&
    match.type >= 0.9 &&
    match.distanceKm <= Math.min(1.5, nearRadius / 1000) &&
    match.confidence >= 0.75;

  return !(namedPlausible || unnamedPlausible);
}
...
function scoreCandidate(water, candidate, anchor, scoreRadiusM = radiusM) {
  ...
  const distance = Math.max(0, 1 - distanceKm / Math.max(1, scoreRadiusM / 1000));
...
function chooseCandidate(water, candidates, anchor, scoreRadiusM = radiusM) {
  ...
  const ranked = [...unique.values()]
    .map((c) => scoreCandidate(water, c, anchor, scoreRadiusM))
...
function renderTs(records) {
  ...
  return `// AUTO-GENERATED by scripts/import-local-water-v3-1-fast.mjs
// Source: ${SOURCE}
...
console.log("");
console.log("HarzFishing – Locality-first Water Matcher v3.1 FAST");
console.log("-----------------------------------------------------");
console.log(`LAV gesamt: ${catalog.length}`);
console.log(`Bearbeitung: ${start}–${start + work.length - 1}`);
console.log(`Schnellsuche: ${nearRadiusM} m -> bei Bedarf ${radiusM} m`);
console.log(`Overpass-Timeout: ${overpassTimeoutSec} s; 1 Versuch je Server`);
console.log("");
...
for (const water of work) {
  ...
  let candidates = [];
  let match = { status: "unmatched" };
  let usedRadiusM = nearRadiusM;

  try {
    const nearElements = await overpassNearby(anchor, water, cache, nearRadiusM);
    candidates = nearElements.map(toCandidate).filter(Boolean);
    match = chooseCandidate(water, candidates, anchor, nearRadiusM);

    if (nearRadiusM < radiusM && shouldExpandSearch(match, candidates, nearRadiusM)) {
      usedRadiusM = radiusM;
      const wideElements = await overpassNearby(anchor, water, cache, radiusM);

      const merged = new Map();
      for (const candidate of [
        ...candidates,
        ...wideElements.map(toCandidate).filter(Boolean),
      ]) {
        merged.set(candidate.id, candidate);
      }

      candidates = [...merged.values()];
      match = chooseCandidate(water, candidates, anchor, radiusM);
    }
  } catch (error) {
    console.warn(`Overpass abgebrochen: ${error.message}`);
    // Bereits gefundene Nahbereichs-Kandidaten trotzdem verwerten.
    if (candidates.length) {
      match = chooseCandidate(water, candidates, anchor, usedRadiusM);
    }
  }
  ...
  console.log(
    ...
    ` [R${Math.round(usedRadiusM / 100) / 10}km]` +
