...
import type { FishingSpot, FishingWater } from "@/lib/types";
...
function FitMarkers({ waters, spots }: { waters: FishingWater[]; spots: FishingSpot[] }) {
  const map = useMap();
  useEffect(() => {
    const coordinates = [...waters.map((w) => [w.latitude, w.longitude] as [number, number]), ...spots.map((s) => [s.latitude, s.longitude] as [number, number])];
...
export default function MapView({ waters, spots, onSelect }: { waters: FishingWater[]; spots: FishingSpot[]; onSelect: (water: FishingWater) => void }) {
  return (
    <MapContainer center={[51.72, 11.1]} zoom={9} scrollWheelZoom className="map">
      <TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
      <FitMarkers waters={waters} spots={spots} />
      {waters.map((water) => (
        <Marker key={water.id} position={[water.latitude, water.longitude]} icon={waterIcon} eventHandlers={{ click: () => onSelect(water) }}>
          <Popup><strong>{water.name}</strong><br />{water.module}<br />{water.fish.join(" · ")}</Popup>
        </Marker>
      ))}
      {spots.map((spot) => (
        <Marker key={spot.id} position={[spot.latitude, spot.longitude]} icon={spotIcon}>
          <Popup><strong>{spot.name}</strong><br />{spot.tags.join(" · ")}<br />{spot.note}</Popup>
        </Marker>
      ))}
    </MapContainer>
  );
