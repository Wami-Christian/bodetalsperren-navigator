...
export type ScoreResult = { score:number; reasons:string[]; waterTemperature:number; dayPhase:string; moonPhase:string; moonIllumination:number };
...
function moonInfo(date:Date){const syn=29.53058867; const known=Date.UTC(2000,0,6,18,14); const age=((((date.getTime()-known)/86400000)%syn)+syn)%syn; const phase=age<1.85||age>27.7?'Neumond':age<7.38?'zunehmend':age<9.23?'erstes Viertel':age<14.77?'zunehmend':age<16.61?'Vollmond':age<22.15?'abnehmend':age<24?'letztes Viertel':'abnehmend'; const illumination=Math.round((1-Math.cos(2*Math.PI*age/syn))/2*100); return {age,phase,illumination};}
...
export function calculateAutomaticFishingScore(water:FishingWater, fish:Fish, hour:ForecastHour):ScoreResult {
 if(!waterHasTargetFish(water, fish)) return {score:0,reasons:[],waterTemperature:0,dayPhase:'',moonPhase:'',moonIllumination:0};
 ...
 return {score:Math.max(0,Math.min(100,Math.round(score))),reasons,waterTemperature:wt,dayPhase,moonPhase:moon.phase,moonIllumination:moon.illumination};
}
